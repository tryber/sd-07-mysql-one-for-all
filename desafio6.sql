...
(SELECT CAST(CONVERT((AVG(SpotifyClone.plans.price)), DECIMAL(6,2)) AS CHAR) AS "faturamento_medio" FROM SpotifyClone.plans join SpotifyClone.users on SpotifyClone.users.plan_id = plans.id,
