SELECT
(SELECT ROUND(MIN(SpotifyClone.plans.price), 2) AS "faturamento_minimo" FROM SpotifyClone.plans join SpotifyClone.users on SpotifyClone.users.plan_id = plans.id) AS "faturamento_minimo",
